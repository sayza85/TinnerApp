...
export class UploadPhotoComponent {
  ...
  async onImgPicked(event: Event) {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (fileType && this.acceptedImageType.includes(fileType.mime)) { // image/jpeg or image/png only
        ...
        this.imgFile = undefined
        this.errMessage.set('Image file must be in JPEG or PNG format') //ตรวจสอบไฟล์รูปภาพว่าเป็นไฟล์รูปภาพหรือไม่
      }
    }
  }
}
